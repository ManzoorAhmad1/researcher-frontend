...
const BookmarkView = ({
  ...
  const handleBookmarkClick = (e: React.MouseEvent) => {
    ...
    // Check if clicking on the main bookmark item content (not on interactive elements)
    ...
    // Only navigate if clicking on the main bookmark item content AND no sidebar is open
    if (isMainContent && !hasOpenSidebar) {
      router.push(`${pathName}/bookmark/${item?.id}`);
    } else {
      e.stopPropagation();
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="h-[150px] rounded-xl mb-2 flex items-center justify-center overflow-hidden">
        {item?.image_url ? (
          <OptimizedImage
            src={item?.image_url}
            alt=""
      ...
      {item.tags && item.tags.length > 0 ? (
        ...
        <span className="text-muted-foreground bg-slate-200 px-2 py-1 mt-2 text-sm rounded-lg ">
          No Tag
        </span>
      )}
