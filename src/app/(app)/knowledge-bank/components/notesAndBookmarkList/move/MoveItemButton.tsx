...
import { moveItemAPIForKB } from "@/apis/notes-bookmarks";
...
export const MoveItemButton: React.FC<MoveDialogProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await moveItemAPIForKB({data: updatedData[0]}, projectId, user?.id);
      if (fetchFolders) {
        fetchFolders();
      }
      ...
      >
        ...
        <div className="px-6 py-4 border-b focus:outline-none">
          <h2 className="dark:text-white text-xl font-semibold text-gray-800">
            Move {itemType === 'note' ? 'Note' : itemType === 'file' ? 'Bookmark' : 'Folder'}
          </h2>
        </div>
        {isMoveDataLoading ? (
