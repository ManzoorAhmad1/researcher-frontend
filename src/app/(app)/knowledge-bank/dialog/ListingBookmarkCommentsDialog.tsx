...
import { Comments, ListingBookmarkCommentsDialogProps } from "../utils/types";
...
const ListingBookmarkCommentsDialog: React.FC<
  ListingBookmarkCommentsDialogProps
> = ({ quill, isOwner, show, setShow }) => {
  ...
  const fetchCommentList = async () => {
    setLoading(true);
      ...
      const response = await getAllBookmark(doc_id);

    if (response.isSuccess == false) {
      console.error("Error fetching CommentList:", response?.message);
    } else {
  ...
  const handleDeleteComment = async (id: string) => {
    ...
    setCommentList(updatedComments);
      ...
      const response = await updateBookmarks(doc_id, {comments:updatedComments});
      ...
      <DialogContent className="max-w-[800px]">
        <div>
          ...
          {loading ? (
            ...
            <ul className="max-h-[calc(100vh-130px)] overflow-auto">
...
export default ListingBookmarkCommentsDialog;
