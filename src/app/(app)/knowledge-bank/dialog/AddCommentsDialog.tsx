...
import { AddCommentsDialogProps } from "../utils/types";
import { v4 as uuidv4 } from "uuid";
import { getNoteByDocId, updateNote } from "@/apis/notes-bookmarks";

const AddCommentsDialog: React.FC<AddCommentsDialogProps> = ({
  show,
  setShow,
  commentsData,
}) => {
  ...
  const handleSubmit = async () => {
    ...
    const response = await getNoteByDocId(doc_id);
    ...
    if (existingData && existingData?.comments?.length > 0) {
      ...
      if (response?.isSuccess == false) {
        ...
        const response = await updateNote(doc_id, {comments: [commentsBody]});
