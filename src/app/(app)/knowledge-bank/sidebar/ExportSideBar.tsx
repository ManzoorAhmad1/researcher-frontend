"use client";
import { Sidebar } from "primereact/sidebar";
import React, { useEffect, useRef, useState } from "react";
import {
  customHeader,
  customIcons,
} from "@/components/Sidebar(Drower)/customHeader";
import { Button } from "@/components/ui/button";
import { ExportSideBarProps, Tags } from "../utils/types";
import { exportDatas } from "../utils/const";
import { getNote } from "@/apis/notes-bookmarks";
import htmlDocx from "html-docx-js/dist/html-docx";
import { LoaderCircle } from "lucide-react";
import { jsPDF } from "jspdf";
import NotesPdf from "./NotesPdf";

// NOTE: Critical performance optimization
// These PDF libraries cause high memory usage during build
// See next.config.js for the optimizations

const ExportSideBar: React.FC<ExportSideBarProps> = ({
  shareId,
  visible,
  setVisible,
}) => {
  const [selectedExportData, setSelectedExportData] = useState("radio-0");
  const [loading, setLoading] = useState(true);
  const [singleData, setSingleData] = useState<any>({});

  const handleChange = (value: string) => {
    setSelectedExportData(value);
  };

  const downloadDocx = async (filename: any) => {
    const doc = htmlDocx.asBlob(singleData?.description);
    const url = URL.createObjectURL(doc);
    const a = document.createElement("a");
    a.href = url;
    a.download = filename;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  };

  const generatePDF = () => {
    try {
      const doc = new jsPDF();
      const content = singleData?.description || "";

      // Add content to PDF
      doc.setFontSize(12);
      doc.setFont("helvetica", "normal");

      // Split content into lines and add to PDF
      const lines = content.split("\n");
      let yOffset = 30;
      const lineHeight = 10;
      const pageWidth = doc.internal.pageSize.getWidth();
      const maxWidth = pageWidth - 2 * 30;

      lines.forEach((line: any) => {
        if (yOffset > doc.internal.pageSize.getHeight() - 30) {
          doc.addPage();
          yOffset = 30;
        }

        const splitLines = doc.splitTextToSize(line, maxWidth);
        doc.text(splitLines, 30, yOffset);
        yOffset += splitLines.length * lineHeight + lineHeight;
      });

      doc.save("ResearchCollab-note.pdf");
    } catch (error) {
      console.error("Error generating PDF:", error);
    }
  };

  const handleDownload = (msg: any) => {
    if (selectedExportData === "MS Word") {
      downloadDocx(`ResearchCollab-note.docx`);
    } else if (selectedExportData === "PDF") {
      generatePDF();
    }
  };

  const colors = [
    { color: "#E9222229", borderColor: "#E92222" },
    { color: "#F59B1429", borderColor: "#F59B14" },
    { color: "#F5DE1429", borderColor: "#F5DE14" },
    { color: "#079E2829", borderColor: "#079E28" },
    { color: "#D4157E29", borderColor: "#D4157E" },
    { color: "#0E70FF29", borderColor: "#0E70FF" },
    { color: "#8D17B529", borderColor: "#8D17B5" },
  ];

  const compareColor = (color: string) => {
    const matchedColor = colors.find((c) => c.color === color);
    if (matchedColor) {
      return matchedColor?.borderColor;
    } else {
      return color.slice(0, -2);
    }
  };

  useEffect(() => {
    const fetchNote = async () => {
      const data = await getNote(shareId);
      if (data?.success) {
        setSingleData(data?.data);
        setLoading(false);
      }
    };

    fetchNote();
  }, [shareId, visible]);

  return (
    <Sidebar
      header={customHeader("Export")}
      icons={customIcons}
      className="bg-white p-6 w-[25rem] dark:bg-[#3A474B]"
      style={{ boxShadow: "-2px 0px 6px 0px #00000040" }}
      visible={visible}
      position="right"
      onHide={() => setVisible(false)}
    >
      <hr className="my-4 dark:border-[#BEBFBF]" />
      {loading ? (
        <div className="h-[calc(100%-33px)] flex justify-center items-center">
          <LoaderCircle className="animate-spin h-10 w-10 mx-auto" />
        </div>
      ) : (
        <>
          <h4 className="text-[#0E70FF] mb-2 font-semibold text-[18px]">
            {singleData?.title}
          </h4>
          <p
            className="text-[#666666] mb-3 dark:text-[#BEBFBF] line-clamp-4"
            dangerouslySetInnerHTML={{ __html: singleData?.description }}
          />
          <div>
            <h6 className="text-[12px] text-[#666666] dark:text-[#BEBFBF] mb-1">
              TAGS
            </h6>
            {singleData?.tags && singleData?.tags?.length > 0 ? (
              <div>
                <div className="flex">
                  {singleData?.tags?.map((tag: Tags, index: number) => (
                    <div
                      key={index}
                      style={{
                        backgroundColor: tag.color && tag.color,
                        color: compareColor(tag.color),
                      }}
                      className="inline-block px-2 py-1 me-1 my-1 whitespace-nowrap text-sm rounded-lg cursor-pointer"
                    >
                      {tag.name}
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <span className="text-muted-foreground bg-slate-200 px-2 py-1 mt- text-sm rounded-lg ">
                No Tag
              </span>
            )}
          </div>
          <hr className="my-4 dark:border-[#BEBFBF]" />
          {exportDatas?.map((item, i) => (
            <div
              key={i}
              className="cursor-pointer"
              onChange={() => handleChange(item.name)}
            >
              <div className="flex gap-2">
                <input
                  type="radio"
                  name="exportDataGroup"
                  id={`radio-${i}`}
                  disabled={item?.disabled}
                />
                {item.icon}
                <label htmlFor={`radio-${i}`}>{item.name}</label>{" "}
              </div>
              <hr className="my-4 dark:border-[#BEBFBF]" />
            </div>
          ))}
          <div className="flex gap-2 justify-end items-center py-4">
            <Button
              onClick={() => setVisible(false)}
              className="rounded-[20px] text-[#0E70FF] border-[#0E70FF] h-9 dark:bg-transparent hover:bg-transparent hover:text-[#0E70FF]"
              variant={"outline"}
            >
              Cancel
            </Button>
            <Button
              onClick={handleDownload}
              className=" rounded-[26px] btn text-white h-9"
            >
              Export
            </Button>
          </div>
        </>
      )}
    </Sidebar>
  );
};

export default ExportSideBar;
