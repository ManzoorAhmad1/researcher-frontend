...
const NetworkChart = () => {
  ...
  const project_id = useSelector((state: any) => state?.project?.project?.id);
  ...
  const processChartData = (data: any[], value: string) => {
    const allChartData = groupPdfsByKeywords(data, value).filter(
  ...
  const groupPdfsByKeywords = (data: Pdf[], value: string): GroupedPdfs[] => {
    ...
    data.forEach((pdf) => {
      ...
      const citation = [pdf?.CitationCount];
  ...
  const handleNodeClick = (node: any) => {
    if (!(node?.data.parent_id == 0 || node.data.parent_id == undefined)) {
      router.push(`/info${slug_id}/${node?.data.id?.split("/")?.[0]}`);
    }
  };
  ...
  const fetchData = async () => {
    try {
      ...
      if (data && data?.length > 0) {
        setApisData(data);
        processChartData(data, dropdownValue);
      } else {
        setChartData([]);
        setApisData([]);
      }
