...
import { CornerRightUp, Layers, LoaderCircle } from "lucide-react";
import { useState } from "react";
import { Dialog, DialogContent, DialogHeader } from "@/components/ui/dialog";
import { startContentInclution } from "@/apis/research-assistant";
import { Tooltip } from "rizzui";
import { ShowMoreDialog } from "./ShowMoreDialog";
import PublicOutlinedIcon from "@mui/icons-material/PublicOutlined";
import DescriptionOutlinedIcon from "@mui/icons-material/DescriptionOutlined";

export function ContentInclutionTableDialog({
  papersData,
  sourceData,
  mainTitle,
  sourceId,
  ...
  const combinedLinks = [
    ...(papersData?.map((item: any) => ({
      title: item?.title || "Untitled",
      link: item?.openAccessPdf?.url || null,
      type: "paper",
    })) || []),
  ...
  const startOCR = async () => {
    setLoading(true);
    try {
      const response = await startContentInclution(sourceId, {
        ...
        >
          <div>
            <DialogHeader className="mb-3 flex justify-center items-center text-2xl">
              {allData?.length > 0 && DataLoaded
                ? "Papers Evaluation Table"
                : "Enriched Insight Sources"}
            </DialogHeader>
            {DataLoaded ? (
