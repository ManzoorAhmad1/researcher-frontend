...
import ProjectDrawer from "../myprojects/ProjectDrawer";
...
const ProjectInfo = () => {
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on("projectDeleted", (projectDeleted: any) => {
        const isDeletedProjectExists = currentWorkspace?.allWorkspaces?.find(
          (workspace: any) =>
  ...
  const toggleTemplates = (id: string) => {
    setToggledTemplates((prev: any) => {
      if (prev.includes(id)) {
        return prev.filter((templateId: any) => templateId !== id);
      } else {
        return [...prev, id];
      }
    });
  ...
  return (
    <div className="mt-8">
      {loading || !projects || projects?.length === 0 ? (
        ...
        <div className="flex max-w-[85%] mx-auto justify-center">
                            ...
                            <div className="flex -space-x-4">
                              ...
                              {(item?.lead_user_id === user?.id ||
                                ...
                                <button
                                  ...
                                  className="w-[36px] h-[36px] flex z-10 items-center text-2xl justify-center border border-[#D5D5D5]  bg-white  text-[#007EEF] rounded-full"
                                >
                                  +
                                </button>
                              )}
                                  ...
                                  index < 2 && (
                                      ...
                                      className={`rounded-[22px] py-4 px-6 my-2 flex justify-between items-center ${
                                        currentProject?.project?.id === item?.id
                                          ...
                                          (  data.type === "Uploaded"||
                                            ...
                                            data?.type ==="Paper Uploaded Via Search"||
                                            data.type === "User Upload" || data.type === "Note Saved") && data?.file_id
                                          ? "cursor-pointer"
                                          : ""
                                      }`}
                                      onClick={() =>
                                        
                                        (data.type === "Uploaded" || data.type === "Paper Uploaded" ||
                                          data.type === "Paper Added Via Search" || data?.type ==="Paper Uploaded Via Search"||data.type === "User Upload" || data.type === "Note Saved") &&
                                        data?.file_id
                                          ? router.push(
                                              `/info/${data?.file_id}`
                                            )
                                    ...
                                    >
                                      <div className="flex gap-3 items-center">
                                        <div>
                                          <div className="border w-[32px] h-[32px] bg-secondaryBackground border-tableBorder rounded-full  flex justify-center items-center">
                                            {data.type === "Paper Added Via Search" || data?.type ==="Paper Uploaded Via Search" ? (
                                              <OptimizedImage
                                                src={
                                              ...
                                              />
                                            ) : data.type === "Paper Uploaded" || data.type === "User Upload" || data.type === "Note Saved" ? (
                                              <File
                                                width={20}
                                                height={20}
                                                color="#1776ff"
                                        ...
                                        <div>
                                          <TooltipProvider>
                                            <Tooltip>
                                              <TooltipTrigger asChild>
                                                <p className={`text-[13px] font-medium text-darkGray ${
                                                  (data.type === "Uploaded" ||
                                                  data.type === "Paper Uploaded" ||
                                                  data.type === "Paper Added Via Search" ||
                                                  data?.type === "Paper Uploaded Via Search" ||
                                                  data.type === "User Upload" || data.type === "Note Saved" || data.type === "Deleted")
                                                  ? `max-w-[200px] truncate ${!data?.file_id  && data.type !== "Note Saved"? "line-through" : ""}`
                                                  : ""
                                                }`}>
                                              ...
                                              {(data.type === "Uploaded" ||
                                                ...
                                                <TooltipContent className="border notification-break border-tableBorder text-left max-h-[150px] overflow-y-auto w-full max-w-[300px] font-size-small z-10 mt-6 rounded-sm bg-headerBackground px-2 py-2 text-darkGray transition-opacity duration-200 group-hover:opacity-100">
                                                  <p className={`font-size-normal font-normal break-words ${
                                                    !data?.file_id  && data.type !== "Note Saved" ? "line-through" : ""
                                                  }`}>
                                                    {data?.activity?.replace(/[^a-zA-Z0-9 \(\)\[\]\{\}\<\>\: ]/g, ' ')}
                                                  </p>
                                                </TooltipContent>
                                          ...
                                          <p className="font-size-small font-medium text-darkGray">
                                              {data.type === "note"
                                                ...
                                                : data.type === "Deleted"
                                                ? "Deleted"
                                                : data.type === "Updated" || data.type === "Status Updated"
                                                ? data.type
                                                : "Uploaded"}{" "}
                                              by
      ...
      <Dialog open={resentActivityOpen} onOpenChange={setResentActivityOpen}>
        ...
        <DialogContent className="fixed p-5  bg-secondaryBackground  z-40  rounded-md shadow-lg top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[500px]">
          <DialogTitle className="text-[18px] font-semibold">
            RECENT ACTIVITY
          </DialogTitle>
          <DialogDescription className="mt-4 text-gray-600 italic text-[0.9rem] ">
                  ...
                  <div key={index}>
                    <div
                      className={`flex justify-between items-start ${
                          (item.type === "Uploaded" || item.type === "Paper Uploaded" ||
                          item.type === "Paper Added Via Search" || item?.type ==="Paper Uploaded Via Search"||item.type === "User Upload" || item.type === "Note Saved") && item?.file_id
                          ? "cursor-pointer"
                          : ""
                      }`}
                      onClick={() =>
                        (item.type === "Uploaded" || item.type === "Paper Uploaded" ||
                          item.type === "Paper Added Via Search" || item?.type ==="Paper Uploaded Via Search"||item.type === "User Upload" || item.type === "Note Saved") &&
                        item?.file_id
                    ...
                    >
                      <div className="flex gap-3">
                        <div className="border w-[32px] h-[32px] bg-secondaryBackground  border-tableBorder rounded-full flex justify-center items-center">
                          {item.type === "Paper Added Via Search" || item?.type ==="Paper Uploaded Via Search"? (
                            <OptimizedImage
                              src={
                                `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/images//blueSearch.svg`
                              }
                              ...
                              className="cursor-pointer "
                            />
                          ) : item.type === "Paper Uploaded" || item.type === "User Upload" || item.type === "Note Saved" ? (
                            <File width={20} height={20} color="#1776ff" />
                          ) : item.type === "Uploaded" ? (
                            <OptimizedImage
                              src={
                                `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/images//blueSearch.svg`
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <p className={`text-[13px] font-medium text-darkGray ${
                                  ...
                                  ? `max-w-[300px] truncate ${!item?.file_id && item.type !== "Note Saved" ? "line-through" : ""}`
                                  : ""
                                }`}>
                              ...
                              {(item.type === "Uploaded" ||
                                ...
                                <TooltipContent className="border notification-break border-tableBorder text-left max-h-[150px] overflow-y-auto w-full max-w-[300px] font-size-small z-10 mt-6 rounded-sm bg-headerBackground px-2 py-2 text-darkGray transition-opacity duration-200 group-hover:opacity-100">
                                  <p className={`font-size-normal font-normal break-words ${
                                    !item?.file_id  && item.type !== "Note Saved" ? "line-through" : ""
                                  }`}>
                                    {item?.activity?.replace(/[^a-zA-Z0-9 \(\)\[\]\{\}\<\>\: ]/g, ' ')}
                                  </p>
                                </TooltipContent>
                          ...
                          <div className="flex gap-1 pt-2 items-center">
                            <p className="font-size-small font-normal text-darkGray ">
                              {item.type === "note"
                                ...
                                : item.type === "Deleted"
                                ? "Deleted"
                                : item.type === "Updated" || item.type === "Status Updated"
                                ? item.type
                                : "Uploaded"}{" "}
                              by{" "}
                              <span className="text-[#0E70FF]">
          ...
          <div className="mt-6 flex justify-end gap-2">
            <Button
              ...
              variant="secondary"
              className="rounded-[26px] h-[32px] text-[13px] font-medium"
            >
              Cancel
            </Button>
          </div>
