...
import { TbListSearch, TbSwitch2 } from "react-icons/tb";
import { CiEdit } from "react-icons/ci";
import { FaCheck } from "react-icons/fa6";
import { RxCross2 } from "react-icons/rx";
import { IoDocumentAttach, IoDocumentTextOutline } from "react-icons/io5";
...
const ProjectInfo = () => {
  ...
  useEffect(() => {
    ...
    if (socket) {
      ...
      socket.on("projectDeleted", (projectDeleted: any) => {
        ...
        )  || currentWorkspace?.allWorkspaces?.find((workspace: any) => workspace?.id === projectDeleted?.workspaceId);
  ...
  const handleNavigate = (item: any) => {
    ...
    item?.file_id
      ...
      : item.type === "note"
      ? router.push(`/knowledge-bank/note/${item?.document_id}`)
      : item.type === "file"
      ? router.push(`/knowledge-bank/bookmark/${item?.document_id}`)
      : null;
  };

  const renderIcon = (data: any) => {
    if (data?.type === "note") {
      if (data?.web_search_id)
        return (
          <MdOutlineScreenSearchDesktop
      ...
      if (data?.creative_thinking_id)
        return <GoLightBulb className="text-[1.3rem]" color="#0F69EE" />;
      if (data?.outline_id)
        return <TbListSearch className="text-[1.3rem]" color="#0F69EE" />;
      if (data?.topic_analysis_id)
        return (
          <MdOutlineLightbulbCircle className="text-[1.3rem]" color="#0F69EE" />
        );
      return (
        <IoDocumentTextOutline className="text-[1.3rem]" color="#0F69EE" />
      );
    }

    if (data?.type === "file") {
      return <FaRegBookmark height={32} width={32} color="#0F69EE" />;
    }
  ...
  return (
    <div>
      <div className="relative top-[-34px] ">
        <div className="w-[100%] max-w-[85%] justify-between mx-auto bg-white dark:bg-[#2C3A3F] border border-[#CCCCCC] dark:border-[#CCCCCC33] box-shadow py-4 rounded-lg px-8">
              ...
              {allActivity.map((item: any, index: number) => {
                ...
                return (
                  <div key={index} onClick={() => handleNavigate(item)}>
                    ...
                    >
                      <div className="w-[79%] flex justify-start gap-2">
                        <div className="border w-[35px] h-[35px] bg-secondaryBackground border-tableBorder rounded-full flex items-center justify-center flex-shrink-0 cursor-pointer">
                          {item.type === "Paper Added Via Search" ||
                          item?.type === "Paper Uploaded Via Search" ? (
                            renderIcon(item)
                          ) : item.type === "Paper Uploaded" ||
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <div className="flex flex-wrap gap-2 pt-2 items-center text-darkGray font-size-small font-normal">
                            <p>
                              {item.type === "note"
                                ? "Added"
                                : item.type === "file"
                              ...
                              by{" "}
                              <span className="text-[#0E70FF]">
                                {item.users?.first_name || user?.first_name}
                              </span>
                            </p>
                            <OptimizedImage
                      ...
                      <div className="text-darkGray text-nowrap text-[11px]">
