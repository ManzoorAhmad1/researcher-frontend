...
import { deleteTeam, handleDeleteMember, updateTeamApi } from "@/apis/team";
...
export const TableSection: React.FC<TableSectionProps> = ({
  ...
  const handleRowClick = (
    ...
    router.push(
      `/account/teams/member?id=${id}&name=${encodeURIComponent(name)}`
    );
  };
  //
    ...
    if (selectedTeam) {
      ...
      try {
        if (selectedTeam?.owner_id === userData?.id) {
          response = await deleteTeam(selectedTeam?.id?.toString());
        } else {
          response = await handleDeleteMember(
        ...
        if (response) {
          toast.success(
            `The team “${selectedTeam?.name}” has been successfully deleted.`
          );
        }
      } catch (error: any) {
  ...
  return (
    <Tabs defaultValue="users">
      <Card className="bg-secondaryBackground border border-tableBorder">
        <CardHeader>
          ...
          <div className="flex items-center mb-4 gap-4">
            <div className="relative flex items-center">
              ...
              <input
                ...
                className="bg-inputBackground border outline-none dark:text-white tableBorder text-foreground text-sm rounded-full block w-full pl-10 p-2.5"
                required
              />
            </div>
          </div>
            ...
            <div className="overflow-x-auto max-w-[1500px]">
              {filteredTeams.length === 0 ? (
                <div className="flex justify-center items-center h-64">
                  ...
                  You have not created any team yet. Begin by creating one.
                  </p>
                </div>
              ) : (
                <Table className="px-4">
                  ...
                  <TableBody>
                    {filteredTeams.map((team: any, index: number) => (
                      ...
                      >
                        <TableCell className="">{team?.teams?.name}</TableCell>
                        <TableCell>
                        ...
                        <TableCell className="">
                          {moment(team?.teams?.created_at).format(
                            "MMMM Do YYYY"
                          )}
                        </TableCell>
                        {allowEdit && (
        ...
        <DialogContent onClick={handleDialogContentClick}>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="rounded-[26px] text-white btn"
