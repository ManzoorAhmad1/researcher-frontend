...
export const MoverModel = ({
  ...
  const titleRender = (node: TreeNode) => {
    return (
      <div
        className={`dark:!bg-[#28363bfa] dark:text-black flex items-center p-2.5 w-full rounded-lg transition-all duration-200 ${
          node.children ? "hover:bg-blue-50 bg-red-50" : ""
        }`}
      >
        {node.children ? (
          <div className="flex items-center">
            ...
            <CiFolderOn className="mr-3 h-5 w-5  text-blue-500 dark:text-yellow-500" />
          </div>
        ) : folderType === "file" ? (
